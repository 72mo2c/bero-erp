// ======================================
// Enhanced New Purchase Return - إرجاع فاتورة مشتريات محسن
// ======================================
...
const NewPurchaseReturn = () => {
  ...
  // Workflow States
  const [currentStep, setCurrentStep] = useState('invoice-info'); // invoice-info, product-selection, details, preview, completed
  ...
  const returnTemplates = {
    ...
    expired: { name: 'إرجاع منتج منتهي الصلاحية', items: [] },
    excess: { name: 'إرجاع كمية زائدة', items: [] }
  };
  ...
  useEffect(() => {
    console.log('🚀 تم تحميل مكون NewPurchaseReturn المحسن');
  ...
  return (
    <ErrorBoundary componentName="إرجاع فاتورة مشتريات محسن">
      <div className="max-w-7xl mx-auto p-4 space-y-6">
        {/* Header with Progress */}
        <div className="bg-white rounded-xl shadow-lg p-6">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-800 mb-2">إنشاء إرجاع فاتورة مشتريات</h2>
              <p className="text-gray-600">فاتورة رقم #{invoice.id} - {supplier?.name || 'غير محدد'}</p>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          {currentStep === 'product-selection' && (
            <div className="space-y-6">
              <div className="text-center mb-6">
                <FaWarehouse className="mx-auto text-4xl text-green-600 mb-4" />
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filteredItems.map((item, index) => {
                      const productIndex = returnItems.indexOf(item);
                        ...
                        <tr key={index} className={`hover:bg-gray-50 ${isDisabled ? 'opacity-50' : ''}`}>
                          <td className="px-4 py-3 text-center">
                            <input
                              ...
                              onChange={(e) => handleItemSelect(productIndex, e.target.checked)}
                              disabled={isDisabled}
                              className="rounded"
                            />
                          </td>
                            ...
                            {item.selected && (
                              <div className="flex gap-2 justify-center">
                                <input
                                  ...
                                  onChange={(e) => handleQuantityChange(productIndex, 'returnQuantity', e.target.value)}
                                ...
                                {item.availableSubQty > 0 && (
                                  <input
                                    ...
                                    onChange={(e) => handleQuantityChange(productIndex, 'returnSubQuantity', e.target.value)}
                                    className="w-16 px-2 py-1 text-xs text-center border border-gray-300 rounded"
                                    min="0"
                          ...
                          <td className="px-4 py-3 text-center">
                            {item.selected && (
                              <select
                                ...
                                onChange={(e) => {
                                  const updated = [...returnItems];
                                  updated[productIndex].condition = e.target.value;
                                  setReturnItems(updated);
                                }}
                          ...
                          <td className="px-4 py-3">
                            {item.selected && (
                              <input
                                ...
                                onChange={(e) => {
                                  const updated = [...returnItems];
                                  updated[productIndex].notes = e.target.value;
                                  setReturnItems(updated);
                                }}
          ...
          {currentStep === 'details' && (
            <div className="space-y-6">
              <div className="text-center mb-6">
                <FaEdit className="mx-auto text-4xl text-purple-600 mb-4" />
                <h3 className="text-xl font-bold text-gray-800">تفاصيل الإرجاع</h3>
                <p className="text-gray-600">أكمل تفاصيل عملية الإرجاع</p>
              </div>
          ...
          {currentStep === 'preview' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {reason === 'defective' ? 'منتج معيب' :
                         ...
                         reason === 'expired' ? 'منتج منتهي الصلاحية' :
                         reason === 'excess' ? 'زيادة في الكمية' :
                ...
                <div className="bg-white p-4 rounded-lg border">
                  <h4 className="font-semibold text-gray-700 mb-3">ملاحظات</h4>
                  <p className="text-sm text-gray-600">{notes || 'لا توجد ملاحظات'}</p>
                </div>
              </div>
          ...
          {currentStep === 'completed' && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6">تم إنشاء إرجاع فاتورة المشتريات وإضافته إلى النظام</p>
